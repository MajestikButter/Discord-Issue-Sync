...
async function handleIssue(issue: GetListedIssueData) {
  ...
  if (!(forum instanceof ForumChannel))
    throw `${forumId} is not a forum channel`;

  await handleIssueForum(issue, forum, DUNGEON_GAME);
}

async function handleIssueForum(
  ...
  if (!threadId) {
    ...
    if (!existing)
      throw `failed to get existing thread ${threadId} for issue ${issue.id}`;
    thread = existing;
  }

  const issueLink = DataFile.getIssueLinkByNumber(issue.number);
  if (!issueLink) {
    const comments = await GitHubApp.getIssueComments(repoInfo, issue.number);
    ...
    const comments = await GitHubApp.getIssueComments(
      repoInfo,
      issue.number,
      DataFile.data.lastGitUpdate
    );
...
async function handleThread(
  ...
  if (opts.msg) {
    const creationDif =
      opts.msg.createdTimestamp - (thread.createdTimestamp ?? 0);
  ...
  if (!issueLink) {
    ...
    const issue = await GitHubApp.createIssue(repoInfo, {
      title: thread.name,
  ...
  if (ownMsg || opts.updateChannel) {
    ...
    if (ownMsg) {
      ...
      if (starter && starter.id == opts.msg?.id)
        body = opts.msg.content ?? undefined;
    }
    const labels = thread.appliedTags
      .map((v) => {
        const d = forum.availableTags.find((t) => t.id == v);
      ...
      .filter((v) => v);
    await GitHubApp.editIssue(repoInfo, issueNumber, {
      labels,
      title: thread.name,
      body,
    });
  ...
  if (opts.updateChannel) {
    const issue = await GitHubApp.getIssue(repoInfo, issueNumber);

    if (issue.locked !== thread.locked) {
      await GitHubApp.setIssueLocked(repoInfo, issueNumber, !issue.locked);
    }
  }
  ...
  if (opts.msg) {
    ...
    const body = `### ${author.username}#${author.discriminator}\n${msg.cleanContent}`;

    switch (opts.msgAction) {
      case "create": {
        await GitHubApp.createIssueComment(repoInfo, issueNumber, body);
        break;
      }
      case "update": {
        ...
        await GitHubApp.editIssueComment(repoInfo, link.gitId, body);
        break;
      }
      case "delete": {
        const link = issueLink.comments.find((v) => v.discordId == msg.id);
        ...
        await GitHubApp.deleteIssueComment(repoInfo, link.gitId);
        issueLink.comments = issueLink.comments.filter(
          (v) => v.discordId != msg.id
        );
        break;
      }
    }
...
Discord.bot.on(Events.MessageDelete, (msg) => {
  if (msg.channel instanceof ThreadChannel)
    handleThread(msg.channel, { msg, msgAction: "delete" }, DUNGEON_GAME);
});

export namespace IssueSync {
  export async function syncFromLastGitHubUpdate() {
    const issues = await GitHubApp.getIssues(
      DUNGEON_GAME,
      DataFile.data.lastGitUpdate
    );
    ...
    await Promise.allSettled(issues.map((v) => handleIssue(v)));
